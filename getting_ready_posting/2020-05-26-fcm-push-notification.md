...
- 디바이스: 우리가 앱을 설치해서 쓰는 디바이스입니다. 아래에서 디바이스 고유 식별자인 fcm 토큰에 대한 이야기를 이어서 하도록 하겠습니다.
...
fcm 프로젝트, 클라이언트 정보가 담긴 `google-service.json` 파일을 `/android/app`에 넣습니다. 아래 파일들을 다음과 같이 수정합니다.
...
심지어 앱이 렌더링되는 시점까지 분리해서 관리해줘야 했지만, 다행히도 
