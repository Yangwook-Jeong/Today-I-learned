...
- lora-network-class
	- class a
		- 평상시 송/수신 모두 꺼둔 상태로 있다가 디바이스가 데이터를 송신할 때, 송신 데이터를 전송한 이후 정해진 시간동안 잠깐 수신 신호를 감지한다.
		- 게이트웨이에서 센서 장치로 제어명령 등 downlink가 있을경우 rit(Receiver Initiated Transmission) 방식으로 처리한다.
	- class b
		- class a가 송신 위주의 전송방식이라면 이 방식은 수신 위주의 서비스에 적합한 전송방식이다.
		- 일정간격마다 주기적으로 신호를 수신할 수 있는 상태가 되고, 해당 시간에 게이트웨이로부터 데이터를 수신할 수 있다.
	- class c
		- 항상 수신이 가능한 상태를 유지하는 방식이다.
		- 때문에 타 class에 비해 최소 지연시간을 갖지만 가장 많은 전력을 소비한다.
		- 충분한 전력이 공급되는 상황에서 고려해야 하며, 원격제어 등 실시간 서비스 구현에 적합한 전송방식이다.

- ml
	- algorithms
		- supervised: 한 세트의 사례들을 기반으로 예측을 수행한다. 과거 매출이력을 이용해 미래 가격을 추산할 수 있다. 지도학습에는 기존에 이미 분류된 학습용 데이터로 구성된 입력변수와 원하는 출력변수가 수반된다. 알고리즘을 이용해 학습용 데이터를 분석해 입력 변수를 출력변수와 매핑시키는 함수를 찾을 수 있다. 추론된 함수는 학습용 데이터로부터 일반화를 통해 알려지지 않은 새로운 사례들을 매핑하고, 눈에 보이지 않는 상황 속에서 결과를 예측한다.
			- classification
				- 범주형 변수를 예측하기 위해 데이터를 사용할때 지도학습을 분류라고 부른다. 
				- 이미지에 강아지나 고양이와 같은 레이블이나 지표를 할당하는 경우에 해당된다. 
				- 레이블이 2개인 경우 binary classification이라고 부르며 범주가 2개 이상인 경우에는 multi-lcass classification이라고 부른다.
			- regression: 연속 값을 예측할때 문제는 회귀 문제가 된다.
			- forecasting
				- 과거나 현재 데이터를 기반으로 미래를 예측하는 과정이다.
				- 예측은 동향을 분석하기 위해 가장 많이 사용한다.
				- 예를 들어 올해와 전년도 매출을 기반으로 내년도 매출을 추산하는 과정이다.
		- semi-supervised: 지도학습은 데이터 레이블링 작업에 많은 비용과 시간이 소요될 수 있다는 다넘이 있다. 따라서 분류된 자료가 한정적일때는 지도하습을 개선하기 위해 미분류 사례를 이용할 수 있다. 이때 기계는 온전히 지도받지 않기때문에 준지도라고 표현한다. 학습 정확성 개선을 위해 미분류 사례와 함께 소량의 분류 데이터를 이용한다.
		- unsupervised: 기계는 미분류 데이터만을 제공받는다. 기계는 clustering structure, low-dimensional manifold, sparse tree and graph 등과 같은 데이터의 기저를 이루는 고유패턴을 발견하도록 설정된다.
			- clustering
				- 특정 기준에 따라 유사한 데이터 사례들을 하나의 세트로 그룹화한다.
				- 이 과정은 종종 전체 데이터 세트를 여러 그룹으로 분류하기 위해 사용한다.
				- 사용자는 고유한 패턴을 찾기 위해 개별 그룹 차원에서 분석을 수행할 수 있다.
			- dimension reduction
				- 고려 중인 변수의 개수를 줄이는 작업이다.
				- 많은 애플리케이션에서 원시 데이터는 아주 높은 차원의 특징을 가진다.
				- 이 때 일부 특징들은 중복되거나 작업과 아무 관련이 없다.
				- 따라서 차원수를 줄이면 잠재된 진정한 관계를 도출하기 용이해진다.
		- reinforcement: 환경으로부터의 피드백을 기반으로 행위자의 행동을 분석하고 최적화시킨다. 기계는 어떤 액션을 취해야 할지 듣기보다는 최고의 보상을 산출하는 액션을 발견하기 위해 서로 다른 시나리오를 시도한다. trial and error와 delayed reward는 다른 기법과는 구별되는 강화 학습만의 특징이다.

- refactoring: 겉으로 드러나는 기능은 그대로 둔채 소프트웨어 내부를 수정하는 작업을 말한다. 더 이해하기 쉽고 수정하기 쉽게 만드는 것이다. 
	- duplicated code: 한 클래스 내에 존재하는 중복된 부분은 하나의 메서드로 추출해야한다. 
	- long method: 메서드를 작은 단위로 과감하게 쪼개야한다. 주석 대신 메서드를 작성하고 주석을 단 코드를 넣는다. 짧은 메서드를 이해하기 쉽도록 메서드 명을 정해야 한다. 메서드 명은 기능 수행방식이 아니라 목적, 기능 자체를 나타내는 이름으로 정한다.
	- large class: 기능이 지나치게 많은 클래스는 엄청난 수의 인스턴스 변수가 있다. 접두/접미어가 같다면 하나의 클래스로 추출하는 것이 좋다. 인스턴스 변수를 계속해서 모두 사용하지 않는 크랠스에 대해서는 클래스/모듈 추출을 적용한다.
	- long parameter list: 매개변수가 많아지면 서로 일관성이 없어지거나 사용하기 불편해지고 더 많은 데이터가 필요해질 때마다 계속 수정해야 하기때문에 매개변수들을 이해하는 것이 어려워진다. 이 경우 객체를 통째로 전달하거나 매개변수를 메서드로 전환해 전달하는 방법을 사용한다.
	- 수정의 산발(divergent change): 한 클래스가 자주 수정될때마다 발생한다. 이럴 경우 하나의 클래스를 여러 개의 변형 객체로 분리하는 것이 좋다.
	- 기능의 산재(shoutgun surgery): 수정할 때마다 여러 클래스에서 수많은 부분을 고쳐야 한다면 이 부분을 의심해본다. 이럴 경우 메서드/필드를 하나의 클래스로 추출한다. 기존의 클래스 중 적절한 클래스가 없다면 새로운 클래스를 만들어 추출한다.
	- 잘못된 소속(feature envy): 객체의 핵심은 데이터와 그 데이터에 사용하는 프로세스를 한데 묶는 기술이다. 다른 객체의 데이터에 접근하기 위해 읽기 메서드를 자주 호출한다면 이 부분을 의심해야 한다. 이 경우도 마찬가지로 메서드를 적당한 클래스로 이동시켜야 한다.
	- 데이터 뭉치(data clumps): 데이터들은 주로 다른 데이터들과 그룹을 이룬다. 데이터 뭉치가 필드처럼 보이는 부분을 찾고 그 부분을 별도의 클래스로 추출한다. 자연스럽게 매개변수들이 객체를 통쨰로 전달하는 방법으로 전달된다.
	- 강박적 기본타입 사용(primitive obsession): 데이터 값들을 객체로 전환한다.
	- switch statement: 대부분의 조건문의 switch는 if로 변경한다.
	- 평생 상속 계층(parallel inheritance hierachies): 한 클래스의 하위클래스를 만들 때마다 매번 다른 클래스의 하위클래스도 만들어야 한다면 이 부분을 의심해야 한다. 이 경우 중복 코드 부분을 제거하기 위해 상속계층의 인스턴스가 다른 상속 계층의 인스턴스를 참조하게 만들어야한다.
	- 직무유기 클래스(lazy class): 리팩토링으로 인해 기능이 축소된 크랠스나 수정할 계획으로 작성했으나 수정을 실시하지 않아 쓸모없어진 클래스를 말한다. 이런 클래스는 비용대비 효율이 떨어지므로 병합하거나 클래스 내용 직접  방법을 통해 제거해야 한다.
	- 막연한 범용 코드(speculative generality): 조만간 이런 기능이 필요하겠다 싶어 추가했으나 아직은 필요없는 기능의 코드를 말한다. 메서드나 클래스가 오직 테스트에서만 사용된다면 의심할 수 있다.
	- temporary field: 어떤 객체 안의 인스턴스 변수가 특정 상황에서만 할당되는 경우가 있다. 복잡한 알고리즘에서 여러 변수를 사용할 때도 이 부분을 의심해야 한다. 
	- message chains: 메시지 체인이란 한 객체에 제 2의 객체를 요청하면 제 2의 객체가 제 3 객체를 요청하는 식으로 연쇄적 요청이 발생하는 것을 말한다. 객체가 어느 대상에 사용되는지 알아내 별도의 메서드로 추출한 후 메서드를 이동시켜야 한다.
	- 과잉 중개 메서드(middle man): 어떤 클래스의 인터페이스가 절반 이상의 기능을 다른 클래스에 위임한다면 원리가 구현된 객체에 직접 접근하는 방식으로 수정한다. 모든 위임을 추적할 필요없이 기능을 확장할 수 있다.
	- 지나친 관여(inappropriate intimacy): 서로 지나치게 관여하는 클래스는 각각 클래스로 분리해 양방향 연결이던 부분을 단방향으로 전환해야 한다. 또는 중개 메서드 역할을 하는 별도의 클래스를 새로 만들어야 한다. 상속으로 인해 지나친 관여가 발생하는 경우가 많은데 이를 위임으로 전환해 해결한다.
	- alternative classes with different interfaces: 기능은 같은데 시그니처가 다른 메서드는 메서드 명을 변경해야 한다.
	- data class: 읽기/쓰기 메서드만 있는 클래스로 대부분 데이터 조작은 다른 클래스가 수행한다. 변경되지 않아야 하는 필드는 private 접근 제어자를 통해 set 메서드를 제한한다.
	- 방치된 상속물(refused bequest): 하위 클래스는 보통 부모 클래스의 메서드와 데이터를 상속받는데 여기 필요한 것을 제외한 나머지는 방치된다. 이는 잘못된 계층 구조때문에 발생하며 메서드/필드 하향을 통해 방치되는 상속물이 없도록 해야한다.
	- comments: 주석이 코드의 구린내를 감추기 위한 탈취제 용도로 쓰이면 안된다. 주석을 넣어야겠다는 생각이 들때는 먼저 코드를 리팩토링하는게 좋다.

## 17.07.2020

- architecture
	- 클라이언트-서버 아키텍처에서 ejb로 대표되는 n-tier model 아키텍처로 진화한 웹은 2000년대 이후부터 비즈니스 요구사항에 빠르게 대처하기 위한 구조로 soa로 진화한다.
- soa
	- 애플리케이션의 거대한 기능을 비즈니스적인 용도로 분류하고 기능 단위로 묶어 하나의 표준 인터페이스를 통해 각각을 서비스로서 조합한다.
	- 구성분류를 아래와 같이 나눌 수 있다.
		- consumer: 서비스의 사용자로 사용자는 실제 클라이언트 또는 다른 서비스가 될 수 있다.
		- provider: 서비스 사용자의 요청에 맞는 서비스를 제공한다. provider는 다른 서비스의 consumer가 될 수 있다.
		- registry: 서비스의 정보를 저장한다. provider가 registry를 통해 consumer에게 서비스를 제공한다.
	- 중요한 것은 provider, consume로 이들은 실제 서비스의 유저뿐 아니라 아키텍처 내부의 모듈도 대상이 될 수 있다.
	- 비즈니스적으로 구분된 서비스들을 느슨하게 연결해 각 컴포넌트를 독립적으로 운용하여 조립이 가능하게끔 한다.
	- 일반적으로 서비스를 위해 서비스를 기능별로 모듈로 분리하고 각 모듈이 다른 모듈과 상호작용할 수 있도록 만들어진 아키텍처를 soa라고 이해하면 된다.
	- 정확히는 soa는 패러다임이며 그를 위한 아키텍처의 초안이다.
	- 모듈의 의존성은 줄이되 모듈 내에 공유할 수 있는 것은 최대한 공유하는 정책을 사용한다.
	- 별도의 플로우를 유지하려한다. 서비스 내에서 결제를 하고자할 때 관련된 루틴을 수행해 결제를 지원함으로써 유저에게 제공하는 서비스를 1차 목적으로 한다. 어느정도 엄격한 프로토콜과 메시지 체계를 운용하게 된다.
	- 서비스들의 재사용에 중점을 둔다. 
...
- msa
	- 마찬가지로 서비스를 독립적으로 운용하며 엔드유저에게 서비스를 제공하는 형태로 이루어진다.
	- soa의 부분집합이라고 할 수 있다.  soa의 패러다임을 따르되 그 아키텍처를 따르지는 않는다.
	- 가능한 공유하지 않고 모듈들이 독립적으로 운용될 수 있도록 아키텍처를 디자인한다.
	- 플로우의 구별을 요구한다. 유저에게 관련된 루틴과 결제 루틴을 별도로 이용하게끔 한다. 즉 서비스 내의 독립이 아닌 자체가 독립된 서비스를 지향한다. 별도의 체계가 없이 경량화된 프로토콜을 통해 운용된다. 
	- 서비스들의 독립을 추구한다.
...
- service planner
	...
	- 서비스 분석을 통해 개선사항 도출 및 서비스 기획
	- 팀 간의 커뮤니케이션, 스케쥴 조율
	- 데이터 및 사용자 리서치 기반으로 한 가설 설정 및 실험 설계 

- file
	- 추상적인 인터페이스이다.
	- path의 유무에 따라 anonymous, named로 나뉜다.
	- data, meta data를 따라관리한다. 프로세스도 마찬가지이다.
- memory
	- path를 가진 backed file의 유무에 따라 anonymous, named로 나뉜다.

## 21.07.2020

- xss(Cross Site Scripting): sql injection과 함꼐 웹상 가장 기초적인 취약점 공격 방법의 일종으로, 악의적인 사용자가 공격하려는 사이트에 스크립트를 넣는 기법을 말한다. 공격에 성공하면 사이트에 접속한 사용자는 삽입된 코드를 실행하며 보통 의도치 않은 행동을 수행하거나 쿠키, 세션 토큰 등의 민감한 정보를 탈취한다. 악성 스크립트를 통해 클라이언트를 공격한다.
	- stored: 해커가 웹서버에 댓글이나 게시글 등에 악성 스크립트가 삽입된 글을 등록하고 그것을 사용자가 열람했을 때 삽입된 악성 스크립트가 실행되어 사용자의 정보를 탈취하는 공격 방법이다.
	- reflected: 악성코드가 삽입된 url을 사용자가 누르게 유도하고 url로 들어가면 스크립트가 실행되는 공격 방법이다.
	- dom: 피해자 브라우저가 html 페이지를 구문 분석할때 공격 스크립트가 dom의 일부로 실행된다. 
- csrf(Cross Site Request Forgery): 악성 스크립트를 통해 서버를 공격한다.

- lazy loading
	- 초기 로딩에서 뷰포트에 보이는 영역의 이미지만 내려받고 나머지는 이미지가 필요한 상황을 판단해 이미지를 내려받는다.
	- 유저의 스크롤이나 기타 요인들을 판단해 이미지를 선택적으로 내려받는 것을 말한다.
	- v75부터는 `iframe`과 `img`에 `loading` attribute에 `lazy`, `eager`를 사용해 선택적으로 lazy loading을 사용할 수 있다.
	- 직접 적용하는 방법은 아래와 같다.
		- 이미지 태그에 `src`를 넣지 않고 `data-src`와 같은 data attribute를 사용해 실제 로드할 수 있는 이미지 주소를 넣는다.
		- 이미지를 모두 읽어 객체에 담는다.
		해당 이미지가 `onload` 상태면 `data-src`에 있는 주소를 `src`에 셋팅시키고 `data-src`는 삭제한다.

- server
	- 접속자가 많은 서비스의 경우 cgi보다 애플리케이션 서버 방식의 처리량이 더 좋다. 여러개의 웹브라우저가 동일한 프로그램을 요청했을 때 cgi는 5개의 요청에 대한 프로그램을 모두 메모리에 적재하지만 애플리케이션 서버 방식은 메모리에 한 번만 적재한다.
	- wsgi(Web Server Gateway Interface)
		- 웹 애플리케이션이 웹 서버와 통신하기 위한 인터페이스이다. 프로토콜로 이해할 수도 있다. 
		- wsgi는 서버와 앱 양 단으로 나뉜다. 
		- wsgi 요청을 처리하려면 서버에서 환경정보와 콜백함수를 앱에 제공해야 한다.
		- 앱은 그 요청을 처리하고 콜백을 통해 서버에 응답한다.
		- gci 디자인 패턴을 기반한 인터페이스이다.
		- wsgi는 cgi가 될 수도 있고 아닐 수도 있다. 
		- http request header를 파싱하고 이를 환경에 추가한다.
		- gunicorn이 python의 wsgi 서버이다. ruby의 unicorn 프로젝트에 영향을 받았다고 한다. http proxy 서버로 nginx를 쓰길 권장한다. 
	- cgi(Common Gateway Interface)
		- 웹 서버에서 애플리케이션을 작동시키기 위한 인터페이스이다.
		- 정적인 웹서버를 동적으로 기능하게 하기 위해 등장했다.
		- 서버 프로그램과 외부 프로그램간의 인터페이스가 바로 이것이다.
		- 근래는 웹 서버의 프로세스로서 인터프리터를 상주시켜 gci로부터 프로그램을 호출하는 부하를 줄였다고 한다.
		- 기존에는 웹서버가 있고 클라이언트에서 외부 프로그램이 필요한 요청이 들어오면 cgi를 통해 외부 프로그램을 실행시켜 응답하도록 했다.
		- 요새는 웹서버에 인터프리터를 내장해 따로 프로세스를 fork해 외부 프로그램을 실행하지 않고 내부에서 다 처리한다.

## 22.07.2020

- javascript
  - array
    - `forEach()`: 실행하는 의미를 갖고 있기 때문에 실행하는 경우 (로그, api 호출 등)에 적합하다.
    - `filter()`: 테스트하는 의미를 갖고 있기 때문에 특정 조건에 해당하는 요소를 뽑아내는 경우에 적합하다.
    - `map()`: 변환하는 의미를 갖고 있기 때문에 각 요소를 변환하는 경우에 적합하다.
  - async/await
    - `await`은 동기식으로 코드를 만들어주기 때문에 주의가 필요하다.
    - `Promise.all`이나 적절한 `await` 배치로 많은 시간을 절약할 수 있다.
    - 독립적인 관계를 가진 함수는 다른 함수에 영향을 주지 않기때문에 동시에 비동기적으로 실행해도 무방하다.
    - 의존적인 관계를 가진 함수는 `await`으로 동기식으로 작성해야 한다.
			```js
			// case 1
			...
			// case 2
			const [book, shop, { key }] = await Promise.all([
				read(param), // 1000ms
			...
			// case 3
			const [book, show, result] = await Promise.all([
				read(param), // 1000ms
				readSomething(param), // 500ms
...
- poc(Proof-of-Concept)
  - poc는 아직 제품/서비스의 형태와는 전혀 상관없는 초보단계에서 어떤 아이디어/방법/이론이 생각한대로 나타나는지 아닌지를 통해 추후에 나타날 위험/실패를 미리 경험/테스트하는 것을 목적으로 한다.
  - 어느 경우에는 실제 개발을 하기 전에 기술적인 가정을 검증하는 과정에 사용한다.
  - 따라서 제품단계의 모습과는 전혀 상관없는 형태로 존재하고, 조직 내부에서만 사용하게 된다.
  - 투자금이 필요한 스타트업인 경우 핵심기술만을 poc의 형태로 만들어 투자자에게 데모를 하는 경우는 가끔 있다.
- prototype
  - poc와 비슷한 테스트 목적이 크다.
  - 다른 점은 poc가 제품/서비스 전체 중의 특정 기능을 테스트하기 위함이라면 프로토타입은 디자인/사용성/기능을 전체적으로 테스트하기 위함이다.
  - mvp가 좀 더 단독제품의 형태를 가졌다면 프로토타입은 아직 draft 상태이다.
	- 실제 시장에 나가지는 않지만 기존 고객이나 잠재고객들의 손에서 테스트한다.
	- 고객들의 피드백과 제품상태에 따라 반복적으로 프로토타입이 개정되고 다시 테스트하길 반복한다.
- mvp(Minimum Viable Product)
  - 제품/서비스는 생존하기에 가능한 최소한의 기능을 포함하고 있다.
	- 시장 가치가 충분히 있는가를 알기 위해 출시한다.
	- 가치를 확인하고, 초기 고객을 확보하고, 생존자금을 마련한 후에 제품 기능을 추가해 릴리즈를 거듭하게 된다.

- nlp
  - 사용할 데이터의 특성에 따라 개발환경에 따라 적합한 형태소 분석기를 고려해야 한다.
  - 연산속도가 중요하다면 mecab를 최우선으로 고려해야 한다. 분석 품질도 상위권이다.
  - 자소 분리나 오탈자에 대해 어느정도 품질이 보장되어야 한다면 komoran을 사용해야 한다.
  - hannanum, khaiii는 일부 케이스에대한 분석 품질이, kkm는 분석 시간에서 아쉬운 부분이 있다.

## 24.07.2020

- gcp
	- app engine: paas이다. 단순히 코드를 배포하기만 된다는 것을 의미한다. 증가된 볼륨을 처리할 인스턴스를 자동으로 생성한다.
		- msa에 적합하며 아래 두 환경을 모두 사용하는 경우 특히 그렇다.
			- 표준환경
				- 샌드박스에서 실행되는 애플리케이션 인스턴스
				- 빠른 확장 처리를 해야하는 애플리케이션
				- 무료나 저렴한 비용으로 실행할 수 있다.
				- 트래픽이 극단적으로 상승함에 따라 즉각적인 확장이 필요한 경우
			- 가변형 환경
				- vm의 docker container 내에서 실행되는 애플리케이션 인스턴스
				- 일관된 트래픽을 수신하거나 정기적인 트래픽 변동을 경험하거나 점진적으로 확장하거나 축소하는 매개변수를 충족하는 애플리케이션
				- 네티이트 코드를 포함하는 프레임워크를 사용하거나 의존하는 경우
				- compute engine 네트워크에 있는 프로젝트으의 리소스나 서비스에 액세스한다.
				- vm 인스턴스가 매주 다시 시작된다. 재시작시 필요한 운영체제, 보안 업데이트를 적용한다.
				- 사용자가 vm 인스턴스에 대한 루트 액세스 권한을 갖는다.
				- cloud build를 사용해 컨테이너 이미지를 빌드할 때 코드 배포에 더 많은 시간이 소요될 수 있다.
	- compute engine: iaas이다. 유연성이 뛰어나고 일반적으로 app engine보다 저렴하다. 단점은 앱과 가상 머신을 직접 관리해야한다는 것이다.

## 27.07.2020

- nmt(Neural Machine Translation): 신경망 기계번역
	- 사람의 뇌가 학습하는 과정을 본딴 기술을 번역에 적용했다.
	- 단어나 구문 단위로 쪼개지 않고 문장 단위로 번역한 결과를 보여준다.
	- 머신러닝이 적용된 엔진을 통해 전체 문맥을 파악한 다음 문장 내에 단어, 순서, 의미, 문맥에서 의미차이를 반영한다.
- smt(Statistical Machine Translation): 통계 기계번역
	- 단어나 구문이 가진 여러가지 의미를 저장해둔다. 일종의 번역사전을 만들어 놓는 것이나 다름없다. 다음으로 사용자가 문장을 입력하면 이를 단어나 구문 단위로 쪼갠 뒤에 통계적으로 가장 본래 의미에 가깝다고 판단하는 결과를 제시한다.
	- 통계적 방법을 쓰는 탓에 night의 밤인지, chestnut의 밤인지 분간해내지 못하는 경우가 적지 않았다.
	- 어순이 다른 점을 읽는 능력이 부족했다.
- rbmt(Rule Based Machine Translation): 룰기반 기계번역
- pbmt(Phrase Based Machine Translation): 구문기반 기계번역

## 28.07.2020

- beacon
	- 역사적 의미로는 적의 침입을 알리는 일종의 군사 연락수단이었다. 육지에서는 적의 습격을 미리 감지해 방어할 수 있는 봉화의 기능을 했고 바다에서는 등대의 역할로 선박의 안전 운행을 유도하는 역할을 했다.
	- it기술과 만나 비콘의 개념이 확장되고 일상 깊숙이 들어왔다. 사물과 상황인식, 컨텐츠 푸시, 실내위치 측위, 자동체크인, 지오펜스 등 다양한 응용서비스 제공이 가능하다.
	- 본질적으로 위치를 알려주는 기준점 역할을 하며 정보전달을 위해 통신기술을 활용한다.
	- 종류
		- ibeacon
			- 비콘의 전통적인 개념만 따왔을뿐 기술적으로는 많이 다르다.
			- 기본 개념을 위치서비스를 ios로 확장한 것이다. 애플이 사용한 기술은 ble이다.
			- 장치의 비콘 신호영역 안에 Ios 기기를 소지한 사용자가 들어오면 애플리케이션에 신호를 보낸다.
			- 예를 들어 상점 근처를 지날때 상점에 설치된 비콘이 할인 쿠폰을 보내주거나 박물관에서 특정 전시물 앞에 가면 관련된 내용을 기기로 보내주는 경우 등이 있다.
		- no-ibeacon
			- alt-beacon: 애플의 규격보다 유연성 있게 재구성한 비콘이다.
			- uri-beacon: uri가 포함된 advertising data를 브라우저를 통해 바로 보여주는 방식이다.
			- place-edge: 갤럭시에 관련 기능을 지원하면서 해당 서비스 앱이 설치되어있지 않아도 이를 바로 연결해주도록 지원한다. 쿠폰, 컨텐츠 등을 전달하는 플랫폼을 별도로 개발하지 않아도 가능하도록 서버 플랫폼을 제공하고 있다.
	- 장점
		- 블루투스 기반의 무선 통신이지만 페어링하는 과정이 따로 필요없다.
		- 저전력 기술이다.
		- ble를 사용해 구현해 ios는 물론 ble를 장착한 모든 기기에서 사용할 수 있다.
		- 최대 70m의 먼 거리까지 사용할 수 있다.
		- 장비에 접촉없이 비콘이 설치된 곳을 지나가기만 해도 데이터전송이 가능하다.
	- 단점
		- 자체 보안성 문제인 spoofing, cloning에 취약하다
			- spoofing: mac, ip주소, 포트 등을 속여 통신 흐름을 왜곡하는 공격이다.
			- cloning: 원본 시스템의 복제본을 하나 이상 생성하는 것으로 기존에 체크인되었던 대사물의 정보를 복제해 그 대상물이 없어도 있는 것처럼 속여서 정보를 빼내는 방법을 말한다.
		- 비콘의 확산으로 많은 채널이 점유되면 비콘 간의 전파 간섭과 충돌로 신호 수신율 저하와 서비스가 불가능해질 수도 있다.
		- 비콘이 부착된 사물이나 소유하는 사용자의 개인정보가 노출될 수 있다.
	- 서비스 종류
		- peble: 매장, 상점을 겨냥한 소형 타입으로 체크인 기능만 있다.
		- marble: 병원이나 공항에서 실내 내비게이션을 가능하게 하는 타입, 체크인과 내비게이션 기능이 있다.
		- nimble: 전시 솔루션이나 박물관에서 적용할 수 있는 타입, 체크인과 내비게이션 기능이 있다.
		- treble: 실외용 위치 솔루션을 위한 타입, 체크인과 내비게이션 기능이 있다.
...
- jwt
  - use-case
    - 회원 인증
      - jwt를 사용하는 가장 흔한 시나리오이다. 유저가 로그인을 하면 서버는 유저의 정보에 기반한 토큰을 발급해 유저에게 전달한다. 그 다음 유저가 서버에 요청할 때마다 jwt를 포함하여 전달한다. 서버가 클라이언트에게서 요청을 받을때마다 해당 토큰이 유효하고 인증됐는지 검증한다.
      - 서버측에서는 유저 세션을 유지할 필요가 없다. 로그인되어있는지 신경쓸 필요가 없고, 유저가 요청을 했을때 토큰만 확인하면 되니 세션 관리가 필요없어 서버자원을 아낄 수 있다.
    - 정보 교류
      - jwt는 두 개체 사이에서 안정성있게 정보를 교환하기 좋은 방법이다. 정보가 sign이 되어있기때문에 정보를 보낸이가 바뀌진 않았는지, 정보가 도중에 조작되지는 않았는지 검증할 수 있다.
  - header
		- 구성
  	  - typ: 토큰의 타입을 지정한다.
      - alg: 해싱알고리즘을 지정한다. 해싱알고리즘으로 보통 hmac 혹은 rsa가 사용되며 토큰을 검증할때 사용되는 signature 부분에서 사용한다.
  			```json
  			{
  				"typ": "JWT",
  				"alg": "HS256"
		...
		- 인코딩
			...
			```
  - payload: 토큰에 담을 정보가 들어있다. 여기에 담는 정보의 한 조각을 클레임이라고 부르는 키/값 쌍으로 이뤄져있다. 토큰에는 여러개의 클레임을 넣을 수 있다.
    - claim
      - registered: 등록된 클레임. 서비스에서 필요한 정보들이 아닌, 토큰에 대한 정보들을 담기 위해 이름이 이미 정해진 클레임들이다. 등록된 클레임의 사용은 모두 optional하며 아래와 같다.
        - iss: issuer. 토큰 발급자
        - sub: subject. 토큰 제목
        - aud: audience: 토큰 대상자
        - exp: expiration. 토큰 만료시간. 시간은 numeric date 형식이어야 하며 현재 시간보다 이후로 설정되어야 한다.
        - nbf: not before. 토큰의 활성 날짜와 비슷하다. 시간은 numeric data 형식이어야 하며 이 날짜가 지나기 전까지는 토큰을 처리하지 않는다.
        - iat: issued at. 토큰을 발급한 시간. 토큰의 age가 얼마나 되었는지 판단할 수 있다.
        - jti: jwt의 고유 식별자로 주로 중복적인 처리를 방지하기 위해 사용한다. 일회용 토큰에 사용하면 유용하다.
...
- jwt
	- payload	
		- claim
      - public: 공개 클레임. 충돌이 방지된 이름을 가지고 있어야한다. 충돌을 방지하기 위해서는 클레임 이름을 uri 형식으로 짓는다.
				```json
				{
					"https://example.com/jwt_claims/is_admin": true
				}
				```
      - private: 비공개 클레임. 클라이언트와 서버 간에 협의 하에 사용하는 클레임 이름들이다. 공개 클레임과는 달리 이름이 중복되어 충돌이 될 수 있으니 사용시에 유의해야한다. 
				```json
				{
					"username": "foo"
				}
				```
      - 아래 예제는 2개의 등록된 클레임, 1개의 공개 클레임, 2개의 비공개 클레임으로 이뤄져있다.
				```json
				{
					"iss": "example.com",
					"exp": "1485000000000",
				...
				```
			- base64로 인코딩하면 아래와 같이 할 수 있다. 주의할 점으로는 `=` 문자가 한 두개 붙을 때가 있다. 이 문자는 base64의 패딩문자라고 부른다. 가끔 url의 파라미터로 전달해야하는 경우가 생기는데 이 `=` 문자는 url-safe하지 않으므로 제거해야한다. 패딩이 한개 생길때도 있고, 두개 생길 수도 있는데 전부 지워야 한다.
				```js
				const payload = {
					"iss": "example.com",
					"exp": "1485000000000",
				...
				```
  - signature: 헤더의 인코딩 값과 정보의 인코딩 값을 합친 후에 주어진 비밀키로 해싱해서 생성한다. 수도코드는 아래와 같다.
		```js
		HMACSHA256(
		...
		```
		- 헤더와 정보의 인코딩 값 사이에 `.`를 넣고 합친다. 이값을 `secret`으로 해싱하고 base64로 인코딩하면 다음과 같은 값이 나온다. 나온 값들을 모두 `.` 구분자로 합쳐주면 하나의 토큰이 완성된다.
			```js
