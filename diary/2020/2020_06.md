...
- lan
	- feature
		- 광대역 전송 매체를 사용해 고속 통신이 가능하다.
		- 경로 선택없이 매체에 연결된 모든 장치로 데이터를 전송한다.
		- 오류 발생률이 낮다.
		- 네트워크에 포함된 자원을 공유한다.
		- 네트워크의 확장이나 재배치가 쉽다.
		- 전송매체로 꼬임선, 동축 케이블, 광섬유 케이블을 사용한다.
	- type
		- bus: 신뢰성과 확장서잉 좋으며 모든 노드들이 버스에 t자형 연결되어 p2p 형태를 가진다. 각 노드들의 연결은 어댑터를 사용하며 양 끝에 terminator를 둔다. 각 노드의 고장은 전체 네트워크에 영향을 미치지 않으며 케이블 사용량이 적다.
		- ring: 통신제어가 간단하고 신뢰성이 높으며 장거리 네트워크에서 사용가능하다. 링 형태이지만 노드 간의 통신은 p2p 형태이다. 각 노드에서 신호 재생이 가능하기때문에 버스 형태와 달리 거리 제약이 적으며 잡음에 강하다.
		- start: 중앙 제어 방식으로 모든 기기가 p2p 방식으로 연결되어있으며 문제해결이 쉽고 하나의 기기의 고장은 전체에 영향을 미치지 않지만, 중앙 제어장비가 고장나면 모든 시스템에 영향을 미친다. 
		- mash: 네트워크 상의 모든 컴퓨터가 연결된 기기나 노드가 고장나더라도 다른 경로를 통해 통신이 가능하며 어떠한 경우에도 네트워크가 동작한다는 장점으 ㄹ가진다. 네트워크에 연결된 기기의 수가 많을 경우 모든 기기와 연결해야 하기때문에 구조가 복잡해진다.

## 29.06.2020

- fp
  - closure: 나주에 사용할 목적으로 인자를 캡처하는 함수이다. 클로저가 캡처한 변수를 자유변수라고 부른다. 클로저는 비공개 접근을 제공할 뿐만 아니라 추상화 기법도 제공한다. 
    ```js
    const add = a => b => a + b
    ...
    ```
  - curry: 2개 이상의 인자로 구성된 함수에 사용한다. 함수의 각 인자를 대응하는 클로저를 반환한다.
    ```js
    ...
    curriedAdd(1)(2) // 3
    ```
  - partial: 부분적으로 실행을 마친 다음 나머지 인자와 함께 즉시 실행한 상태가 되는 함수이다. 1번째 인자에 함수를 받고, 2번째 인자 이후에 기억할 인자를 받는다. 그리고 부분적용의 반환값은 클로저이다. 클로저를 실행하면 부분적용의 1번째 함수에 나머지 인자들을 주입해 실행한다.
    ```js
    ...
    add10(5) // 15
    ```
  - container pattern: 잠재적으로 위험한 코드 주위에 안전망을 설치하는 것이다. 값을 컨테이너화하는 행위는 함수형 프로그램의 기본 디자인 패턴이다. 값을 안정적으로 다루고 불변성을 지키기위해 직접 접근을 차단하는 것이다. 이렇게 감싼 값에 접근하는 유일한 방법은 연산을 컨테이너에 매핑하는 것이다.
    ```js
    const f = x => x + 2
    const g = x => x * x
...
- aop
  - features
    - 프로그램 로직을 분리된 파트로 변화시키는 것을 의미한다. 핵심적인 기능에서 부가적인 기능을 분리해서 aspect라는 모듈로 만들어 설계하고 개발하는 방법이다. 
    - oop를 돕는 보조적인 기술이다. 공통되는 기능들을 여러 모듈에 적용하기 위한 방법으로는 상속이 있다. 다중상속을 지원하지 않는 자바에서는 상속을 통해 기능을 주는데는 한계가 있다. 기능을 핵심 비즈니스 로직과 공통 모듈로 구분하고 핵심로직에 영향을 미치지 않고 사이사이에 공통 모듈을 효과적으로 끼워넣는 방법이다.
  - exmaple
    - 간단한 메서드 성능 검사를 할 수 있다. 개발 도중 다량의 데이터를 넣고 빼는 작업에 대해 시간을 측정하고 쿼리를 개선하는 작업은 아주 중요하다. 해당 작업을 코드 밖에서 구현하고 사용하는 편리 좋기때문에 사용할 수 있다.
    - 트랜젝션 처리를 할 수 있다. 데이터베이스의 상태를 변화시키기 위해 수행하는 작업의 단위로 비즈니스 로직 전후에 설정한다.
  - elements
    - join-point: 모듈의 기능이 삽입되어 동작할 수 있는 실행가능한 특정 위치를 말한다. 
    - point-cut: 어떤 클래스의 어느 조인 포인트를 사용할 것인지 결정하는 기능이다. aop가 항상 모든 모듈의 모든 조인포인트를 사용하지 않기때문에 필요에 따라 사용할 모듈의 트정 조인포인트를 지정할 필요가 있다.
    - advice: 조인포인트에 삽입되어 동작하는 코드를 말한다.
    - intercepter: 어드바이스의 한 종류로 주로 1개의 invoke 메서드를 가진다.
    - weaving/cross-cutting: 포인트컷에 의해 결정된 조인포인트에 지정된 어드바이스를 삽입하는 과정이다. 핵심 모듈 코드에 전혀 영향을 주지 않으면서 필요한 기능을 추가할 수 있게 해준다.
    - aspect: 포인트컷(어디에서) 어드바이스(무엇을 할지)를 표현하는 코드를 말한다.

- 위험관리
  - 프로젝트 초기에 위험을 도출해야 한다. 초기에 식별하고 관리할수록 프로젝트는 안전해진다. 
  - 유사한 프로젝트 수행사례를 확보해야 한다. 위험은 공통적인 요소가 많다. 프로젝트 차원에서 위험목록을 관리해야한다. 발생한 위험과 해결방안을 정리해 구성원과 공유한다. 위험해결 이력을 재사용해 위험요소를 식별 및 해결하기 쉬워진다.
  - 일정산정에 버퍼를 추가해야한다. 언제 발생할지 모르는 위험을 대비해서 프로젝트 일정 산정시 버퍼를 산정해야 한다.
  - 식별된 위험은 계속 추적해야 한다. 위험에 관련된 담당자와 해결목표 일을 정해야한다. 주간회의 등을 통해 위험상태를 정기적으로 감시해야 한다.
  - 위험은 신속하게 공개해야 한다. 이해관계자와 함께 고민하면 쉽게 해결할 수 있다. 관리자는 상황을 판단해 배포일정 연기나 스펙아웃을 결정해야 한다.
