...
- service scalability
  ...
  - cache
    ...
    - 데이터베이스에 쿼리를 하면 캐시에 그 결과 데이터셋을 저장한다. 해싱된 쿼리가 그 캐시의 키가 된다. 그 다음 번에 쿼리를 실행하면 쿼리가 캐시에 존재하는지 확인한다.
...
- message queue
  ...
  - rabbit mq
    ...
    - usage
      ...
      - 시점별 다양성, 요청/응답, 게시/구독 메시지가 필요한 경우 사용한다.
  - kafka: linkedin이 개발했으며 높은 처리량을 요구하는 실시간 데이터 피드 처리나 대기시간이 짧은 플랫폼을 제공하는 것을 목표로해 tcp 기반 프로토콜을 사용한다. 클러스터를 중심으로 producer/consumer가 데이터를 push/pull하는 구조를 가진다.
    - features
      - publish/subscribe 방식
      - 고가용성
      - 확장성
      ...
      - 분산처리
    - usage
      - 메시지가 한번 전달되면 대기열에서 제거되는 전통적인 브로커와는 달리 필요에 따라 이벤트 스트림을 재생할 수 있다.
      - 복잡한 라우팅 없이 a에서 b로 스트리밍, 분할된 순서대로 1회 이상 제공한다.
      - 애플리케이션에서 스트림 히스토리에 대한 액세스 권한이 필요한 경우, 분할된 순서대로 한 번 이사 제공한다.
  - sqs
    - features
      - security: 메시지를 주고받을 수 있는 사용자를 제어하는 기능을 제공한다.
      - durability: 메시지의 안전을 보장하기 위해 여러 서버에 저장해둔다.
      - availability
      - scalability: 버퍼 요청을 독립적으로 처리할 수 있다.
      - reliability: 메시지 처리 중에는 해당 메시지를 잠궈버린다.
      - customization: 큐에 지연시간을 지정한다거나 256kb가 넘는 대규모 메시지를 전송하기 위해 s3나 dynamo db를 사용할 수 있다.
    - lifecycle
      - publisher가 메시지 a를 큐로 전송하고, 이 메시지는 sqs 서버에 중복분산돼서 저장된다.
      - consumersms 메시지를 처리할 준비가 되면 큐에서 메시지를 소비하고 메시지 a를 받는다.
      - 메시지 a는 처리되는 동안 대기열에 그대로 남아있고 제한시간이 초과되면 후속 수신요청으로 반환하지 않는다.
      - consumer는 큐에서 메시지 a를 삭제해 제한시간 초과가 만료되면 메시지가 처리되지 못하도록 한다.

        <img src="https://veluxer62.github.io/assets/images/posts/message-queue/sqs-message-lifecycle-diagram.png" width="500">

    - model
      - standard
        - 무제한에 가까운 tps를 제공할만큼 성능이 좋고 최소 1회 이상의 전송을 보장한다.
        - 일반적으로 메시지가 전송한 순서와 동일한 순서로 전송되도록 최신의 정렬을 제공하지만 순서를 보장하진 못한다.
      - fifo
        - 작업 및 이벤트의 순서가 중요한경우 사용한다.
        - 정확히 1회 처리를 제공하지만 tps의 수가 제한적이다.
        - 일괄처리시 초당 최대 3000개, 개당 처리시 초당 최대 300개까지 가능하다.
    - pricing
      - consumer가 polling 방식으로 메시지를 소비하는 점을 주의해야 한다.
      - api 요청 별로 가격을 책정하므로 메시지가 존재하지 않아도 요청 polling으로 인한 요청비용이 발생한다.
      - 메시지 전송시 최대 64kb까지만 1개의 요청으로 간주한다. 200kb의 메시지를 전송했다고 가정하면, 4개의 송신 요청과 4개의 수신 요청, 1개의 삭제 요청이 발생한다.
      - 비용절감을 위해서는 다음과 같은 방법을 사용할 수 있다.
        - long polling: 메시지가 주기적이지 않고 자주 발생하지 않는다면 최대 20초마다 polling의 횟수를 줄일 수 있다.
        - batch request: 개별 메시지를 일괄처리함으로써 api 요청 횟수를 줄일 수 있다.
          - `sendMessageBatch`
          - `deleteMessageBatch`
  - amazon mq: apache active mq용 관리형 메시지 브로커 서비스이다. 프로토콜이 호환되는 경우 마이그레이션할 때 유용하다.
    - active mq
      - jms(Java Message Service) 클라이언트와 함께 java로 개발한 오픈소스 메시지 브로커이다. 다양한 언어간의 클라이언트/프로토콜을 제공한다.
      - 클러스티링 기능, db, 파일시스템을 통해 각 시스템간의 일관성/지속성을 유지시켜준다.
    - jms
      - java ee기반이며 메시지 생성/송수신/읽기를 하며 또한 비동기적이고 신뢰성과 느슨한 결합을 통해 분산 애플리케이션 컴포넌트간의 통신을 허용한다.
      - 다른 mom와는 통신이 불가능하다.
    - model
      - point to point
        - publisher가 메시지를 큐에 전송하면 각 메시지를 하나의 consumer가 소비하는 것을 보장한다.
        - 큐는 consumer가 메시지를 소비하는 동안 다른 메시지를 만료시까지 보관한다.
        - 만약 등록된 consumer가 없는경우 큐는 consumer가 등록될때까지 보관한다.
      - publish/subscribe
        - publisher가 topic을 발행하고 0개 이상의 subscriber가 구독하며 메시지를 소비하는 형태이다.
        - publisher/subscriber는 타이밍에 의존한다. subscriber는 구독을 시작한 이후에 publisher가 발행한 메시지만 소비할 수 있다.
        - 해당 문제를 해결하기 위해 jms는 durable subscription을 제공한다.
        - 중복처리에 대한 문제가 발생할 수 있는데 virtual destination을 활용해 문제를 해결할 수 있다.

          <img src="https://veluxer62.github.io/assets/images/posts/message-queue/activemq-diagram.png" width="500">

    - pricing
      - 브로커 요금과 브로커 스토리지 요금을 별도로 지불해야 한다.

## 28.05.2020

- css page break
	- `page-break-after`: 특정 단락 직후에 페이지를 넘긴다.
	- `page-break-before`: 특정 단락 직전에 페이지를 넘긴다.
	- `page-break-inside`: 요소 내부에서 페이지를 분리하도록 지정한다. 분리된 페이지는 인쇄시 다음 면에 출력된다. 
		```css
...
- rtmp: Real Time Message Protocol
	- adobe의 독점 프로토콜로 비디오/오디오 등 인터넷 상에서 실시간 스트레밍 데이터 전송해 불특정 다수가 받아볼 수 있도록 하는 기술의 규격이다.
	- 기본으로 1935 포트를 사용하지만 434나 80을 사용해 통신하도록 시도한다.

- cloudfront invalidation
	- 엣지 캐시에서 파일을 무효화한다. 다음에 최종사용자가 파일을 요청하면 최신버전의 파일을 가져오도록 오리진에 반영한다.
	- 파일 버전관리를 사용해 서로 다른 이름을 가진 여러 버전의 파일을 ㅔ공한다.
	- `*`로 와일드카드를 사용하거나 개별 파일에 대한 경로를 지정할 수 있다.
	- 월별로 1000개의 무효화를 무료로 사용할 수 있다.
	- 이전에 생성한 무효화를 복사하고 무효화 경로의 목록을 업데이트하면 업데이트된 무효화를 사용할 수 있다.
	- 하지만 업데이트된 무효화를 실행하지 않고 저장할 수는 없다.
	- 무효화 요청을 하면 각 엣지 로케이션은 즉시 무효화를 처리하기 시작하므로 취소는 불가능하다.

## 29.05.2020

- i/o 모델
  - blocking: 애플리케이션 실행시 os 대기열에 들어가면서 요청에 대한 system call이 완료된 후에 응답을 보낸다.
  - non-blocking: 애플리케이션 실행시 os 대기열에 들어가지않고 실행여부와 관계없이 바로 응답을 보낸다. 바로 응답하기 힘든경우 에러를 반환하는데 정상데이터를 받을때까지 계속해서 요청을 보낸다.
  - synchronous: system call을 기다린다. 시스템의 반환을 기다리는동안 대기열에 머무는 것이 필수는 아니다.
  - asynchronous: system call을 기다리지 않는다. 요청에 대한 처리완료 여부에 관련없이 응답하고 다음 코드를 돌린다. 이후 os에서 처리여부를 알려주고 응답한다.
    
    <img src="https://img1.daumcdn.net/thumb/R1280x0/?scode=mtistory2&fname=http%3A%2F%2Fcfile9.uf.tistory.com%2Fimage%2F99D33B3359841BA52E7E6B" width="500">

  - sync-blocking
    - 프로그램이 블로킹을 일으키는 시스템 함수를 호출한다.
    - 한 작업당 한 번의 사용자-커널 사이의 문맥교환이 발생한다.
    - 정지된 프로그램은 cpu를 사용하지 않고 커널의 응답을 대기한다.
    - 프로그램 관점에서 마치 처리로직이 오래걸리는 것 같지만 사실은 커널의 응답을 기다리느라 블록되어 있는 것이다.

      <img src="https://img1.daumcdn.net/thumb/R1280x0/?scode=mtistory2&fname=http%3A%2F%2Fcfile8.uf.tistory.com%2Fimage%2F99605F3359841BC325C99F" width="500">

  - sync-non-blocking
    - 동기블로킹의 개선안이지만 비효율적이다. 
    - io 지연을 초래한다.

    <img src="https://img1.daumcdn.net/thumb/R1280x0/?scode=mtistory2&fname=http%3A%2F%2Fcfile10.uf.tistory.com%2Fimage%2F992E8B3359841BD8049687" width="500">

  - async-blocking
    - io는 논블로킹이고 알림이 블로킹인 방식이다.
    - `select()` 시스템함수 호출이 사용자 프로세스를 블로킹한다.

    <img src="https://img1.daumcdn.net/thumb/R1280x0/?scode=mtistory2&fname=http%3A%2F%2Fcfile21.uf.tistory.com%2Fimage%2F9965003359841BEA07B781" width="500">
  
  - async-non-blocking
    - system call이 즉시 io 개시여부를 반환한다.
    - 사용자 프로세스는 다른 일을 할 수 있고 io는 백그라운드에서 처리된다.

    <img src="https://img1.daumcdn.net/thumb/R1280x0/?scode=mtistory2&fname=http%3A%2F%2Fcfile3.uf.tistory.com%2Fimage%2F991ACA3359841BFD04521D" width="500">

- nodejs
  - event-loop
    - 메인 겸 싱글스레드로 비즈니스 로직을 수행한다. 수행 도중 블로킹 io 작업을 만나면 커널 비동기 혹은 자신의 워커쓰레드풀에게 넘겨주는 역할을 한다.
    - 웹을 예로 들자면 내부로직은 이벤트루프가 수행하지만, 외부 api를 호출하는 것은 커널 비동기 혹은 자신의 워커쓰레드가 한다.
    - 동시에 많은 요청이 들어온다 해도 1개의 이벤트루프에서 처리한다. 따라서 js로직이 무겁다면 많은 요청을 처리하기 힘들다. 
    - 이벤트루프가 블로킹되는 것은 큰 일이라고 할 수 있다. gc조차 이벤트루프에서 돌고 있으니 이벤트루프가 바쁘면 메모리누수가 발생할 수밖에 없다.
    - 동작과정
      - 이벤트루프는 아래와 같은 phase를 가지고 있다. phase는 각각 fifo 큐를 가지고 있으며, 이 큐에는 이벤트의 콜백을 넣고, cpu가 할당될때 실행한다.
        - timers
          - `setTimeout()`, `setInterval()`과 같은 타이머 콜백들이 처리된다.
          - 타이머 콜백 내부로직은 poll 큐에 먼저 등록된 콜백들이 처리되고 나중에 처리될 수도 있으므로, 파라미터로 지정한 시간에 딱 실행됨을 보장하지 못한다. 즉 파라미터는 일정 시간 후에 실행된다는 기준시간 같은 셈이다.
        - i/o callbacks
          - 클로즈 콜백, 타이머로 스케줄링된 콜백, `setImeediate()`를 제외한 거의 모든 콜백들을 집행한다.
          - 작업완료는 이벤트루프가 io 콜백 영역을 호출할때 체크할 수 있지만, 이후에 poll 큐에 등록되므로 이벤트루프가 poll 영역을 처리할때 콜백 내부로직이 실행된다.
        - poll
          - cpu를 할당받았을 때 `setImmediate()`가 있으면 check로 넘어가고, 없으면 이벤트루프가 phase를 돌며 콜백을 무한히 기다린다.
        - check: `setImmediate()` 콜백은 여기서 호출되고 집행한다.
        - close callback: `.on('close', ...)`같은 이벤트가 여기서 처리된다.
      - 예제1
        - `fs.readFile`이라는 블로킹 작업을 만난 시점에 이벤트루프는 워커쓰레드에게 작업을 넘긴다.
        - 워커쓰레드가 작업을 완료한 뒤 i/o callback 영역의 큐에 콜백을 등록한다.
        - 이벤트루프가 i/o callback 영역을 실행할때 콜백을 poll 영역의 큐에 등록한다.
        - 이벤트루프가 poll 영역을 실행할때 큐에 1개가 있으므로 이것을 실행한다.
        - 콜백 내부에 `setTimeout()`이 있으므로 다시 timers 영역에 넣고 아래 라인을 읽는다.
        - 콜백 내부에 `setImmediate()`가 있으므로 check 영역에 넣는다.
        - 이벤트루프가 poll 큐를 비우고, 다음 실행영역인 check 영역으로 간다. check 영역의 큐에는 들어있는 B를 콘솔에 찍는다. 
        - check 영역의 큐를 비우고 다시 while문의 시작지점으로 간다.
        - 이벤트루프가 timers 영역을 호출한다. uv_run_timers()는 `setTimeout()`의 콜백을 poll 큐에 등록한다.
        - 이벤트루프가 2번째로 poll 영역을 실행한다. 큐에 1개가 있으므로 이것을 실행하고 콘솔에 A를 찍는다.
        - node 프로세스가 반환되고 끝난다.
          ...
          ```
      - 예제2
        - `setTimeout()`을 만나면 timers 영역에 넣고 아래 라인으로 간다.
        - `setImmediate()`를 만나면 check 영역에 넣는다.
        - 프로세스의 사앹에 따라 랜덤하지만 timers가 이벤트루프의 영향을 받으면 먼저 poll 큐에 등록된다.
        - timers의 다음은 poll이라 A가 찍힐 것이다. 하지만 timers를 지나쳤을 경우 check 영역이 호출되므로 B가 찍힌다.
  ...
  - libuv
    - 윈도우/리눅스 커널을 추상화해서 감싸고 있다.
    - nodejs는 libuv 위에서 동작하며, node 인스턴스가 뜰때 libuv는 워커 쓰레드풀(기본 4개)이 생성된다.
    - 위에서 블로킹 작업이 들어오면 이벤트루프가 uv_io에게 내려준다.
    - libuv는 커널단에서 어떤 비동기 작업을 지원해주는지 알고 있으므로 그런 종류의 작업을 받으면 커널의 비동기 함수를 호출한다.
    - 작업이 완료되면 system call을 libuv에게 던져준다.
    - libuv 내에 있는 이벤트루프에게 콜백으로 등록이 된다.

- react-navigation v5
	- `StackNavigator`/`NativeStackNavigator`:  각각의 화면이 스택으로 쌓이는 네비게이션이다.
	- `DrawerNavigator`:  안드로이드에서 생겨났으며 버튼을 눌렀을시 옆에 메뉴 슬라이더를 열었다 닫을 수 있다.
	- `BottomTabNavigator`: ios에서 생겨났으며 아래 버튼들이 있고 버튼을 누를때마다 화면을 전환시킨다.
	- `MaterialTopTabNavigator`: 안드로이드 material 디자인 가이드에 따라 위에 배치되었으며 기본적으로 좌우로 화면을 슬라이드할 수 있다. 화면을 전환할 때마다 indicator들이 따라움직인다.
	- `MaterialBottomTabNavigator`: 해당 화면이 활성화된 경우 버튼 안에 내용물이 커진다. 활성화되지 않은 버튼들은 텍스트가 보이지 않는다.