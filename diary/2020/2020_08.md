...
- typescript
  - generic
    - 타입변수는 부등호 `<>`로 정의한다.
    - 정의한 타입 변수를 요소의 타입 정의에 사용할 수 있다.
    - 컨벤션상 대문자로 시작하며 파스칼 케이스를 사용한다.
      ```ts
      function 함수명<타입변수>(인자타입): 반환타입 {}
      ```
    - 함수를 호출할 때는 정의에서 매개변수에 있던 자리에 인자를 넣는다.
  - intersection
    - 여러 타입을 앰퍼샌드 기호로 이어서 인터섹션 타입을 나타낸다.
    - `A & B` 타입의 값은 A 타입에도, B 타입에도 할당 가능해야한다.
    - A와 B 모두 객체 타입이라면 `A & B` 타입의 객체는 A와 B 타입 각각에 정의된 속성을 모두 가져야 한다.
    - 인터섹션 타입은 몇개든 이어가며 정의할 수 있다.
  - indexable
    - 색인 가능 타입을 정의하기 위해서는 색인에 접근할 때 사용하는 기호인 `[]`를 이용해 객체의 색인 시그니처를 적어줘야 한다.
      ```ts
      interface Foo {
        [key: string]: number | undefined
      }
      ```
    - `[key: string]`을 인덱스 시그니처라고 한다.
    - 색인된 값을 number 타입의 값으로 사용하고 싶다면 먼저 undefined인지 여부를 체크해야 한다.
      ```ts
      const h = Foo['a']
      if (h !== undefined) {
      ...
      ```
    - 색인의 타입으로는 문자열이나 숫자만 사용 가능하다. 문자열, 숫자 색인이 모두 존재하는 경우 숫자로 된 값의 타입은 문자열로 색인된 값 타입의 서브타입이어야 한다. 즉 B타입의 모든 값은 A타입에도 속한다고 이해할 수 있다.
      ```ts
      interface Mixed<A, B> {
        [str: string]: A
        [num: number]: B
      ...
      ```
    - `readonly` 키워드로 읽기 전용으로 선언할 수 있다.
    - 색인 가능 타입을 사용하는 인터페이스 중 하나로 Array가 있다. 만약 모든 색인 가능 타입이 없이 T 타입의 원소를 갖는 Array 인터페이스를 작성한다면 모든 색인에 대한 타입을 일일이 정의해야 하지만 인덱스 타입을 이용하면 손쉽게 해결할 수 있다.
      ```ts
      // as-is
      interface Array<T> {
        length: number
  ...
  - extending
    ...
    - 새로 정의할 인터페이스는 모든 확장 대상 인터페이스의 속성에 자신의 속성을 더한 타입을 갖는다.
      ...
      const macbook: Laptop = { voltage: 220, width: 30, height: 21, color: 'white' }
      ```
    - 확장 대상인 인터페이스 중 여러 인터페이스가 같은 이름의 속성을 가질 수 있다.
    이름이 겹치는 속성의 타입은 모든 확장 대상 인터페이스에서 같아야 한다. 다른 타입을 갖는다면 타입에러가 발생한다.
  - class
    - 속성 이름 뒤에 `!`를 붙여 확정적 할당 단언을 제공할 수 있다. `password: T`를 `password!: T`로 변경할 수 있다. 컴파일러는 해당 타입의 초기화 체크를 건너뛴다.
    - static 속성은 `this.count`따위가 아닌 `Counter.count`와 같은 방식으로 접근할 수 있다.
      ```ts
      class Counter {
      ...
      ```
    - protected는 자식클래스가 부모클래스의 멤버에 접근할 수 있는 권한이 생긴다.
      ```ts
      class User {
      ...
      ```
  - accessor
    - 접근자는 이름 그대로 클래스 속성에 대한 접근할 때 실행될 로직을 정의하는 함수이다.
    - 멤버접근은 크게 두가지로 나뉘고 접근자 역시 두가지가 있다.
      - 읽기접근은 getter이다. getter는 메서드 앞에 `get` 키워드를 붙이며, 속성의 값을 읽어올 때 실행할 함수를 정의할 수 있다. getter는 인자를 받을 수 없고, getter의 반환값은 외부에 해당 속성의 값으로 노출된다. 따라서 해당 멤버를 `readonly`로 인식한다.
        ```ts
        class Shape {
        ...
        const vertices = triangle.vertices // 3
        ```
      - 쓰기접근은 setter이다. 메서드 앞에 `set` 키워드를 붙인다. 새로 할당되는 값을 인자로 받는다.
        ```ts
        class Shape {
        ...
        suqare.vertices = 4
        const vertices = square.vertices // 4 
        ```
  - interface vs. class
    - 값을 가져야 하는 특정한 형태가 있다는 점이 유사하다.
    - 인터페이스 또는 클래스 확장과 유사하게 인터페이스와 클래스 사이에도 연결고리를 만드는 법이 있다.
    - 인터페이스는 본질적으로 값이 어떤 멤버를 반드시 가져야 하고 그 멤버는 타입이 어때야 한다는 제약을 나타내는 수단이다.
    ...
    - 인터페이스가 기존에 존재하는 클래스의 형태를 확장하는 것 또한 가능하다. 인터페이스 확장과 유사하게 `extends` 키워드를 사용해 클래스를 확장할 수 있다.
      ...
      ```
  - assignable
    - number 타입에 OneDigitOdd 타입의 값을 할당할 수 있다. 1, 3, 5, 7, 9는 모두 number에 속한다. 하지만 4는 is not assignable 에러가 발생할 것이다.
      ```ts
      type OneDigitOdd = 1 | 3 | 5 | 7 | 9
      const three: OneDigitOdd = 3
      const num: number = three
      ```
    - 타입스크립트에서는 두 타입의 구조만을 비교해 호환성을 결정한다.
    - 어떤 타입이 다른 타입이 갖는 멤버를 전부 가지고 있다면 그걸로 충분하다.
    - 두 타입이 호환되는 타입이라는 명시적인 표시는 하지 않는다.
    - 이렇게 동작하는 타입 시스템을 구조적 타입 시스템이라고 부른다.
    - 반대되는 개념으로는 노미널 타입시스템이 있다. 노미널 타입 시스템을 갖는 언어는 특정 키워드를 통해 서로 호환가능하다고 명시적으로 표현된 타입간의 할당만 가능하다.
    - 과잉 속성검사는 개발자의 실수를 막기 위해 존재한다. 할당받는 타입에 존재하지 않는 속성이 있다면 타입에러를 띄우는 검사이다.
      ```ts
      interface Color {
        R: number
        G: number
      ...
      const white: Color = someColor // ok
      ```
    - 어떤 타입의 값에 객체 리터럴을 직접 할당하는 경우, 만약 해당 타입에 정의되지 않는 멤버는 오타 등의 실수로 인해 존재할 가능성이 높다고 가정하고 있기때문에 이런 검사를 한다.
    - 아래처럼 딱 한번만 사용할 객체 리터럴에 알려지지 않은 속성이 있다면 대부분 오타이다.
      ```ts
